function mu=solveODE(param,time_resolution)
...
PA.a1 =  param.a1; %4T1 tumour growth parameters
PA.a2 = param.a2;%4T1 tumour growth parameters
PA.d1 =  0;
PA.d2 = param.d2;%4T1 tumour growth parameters
PA.d3 = PA.d2;
...
PA.kappaPrimer =  param.kappaVV; %Infection rate [1/month]   ;
PA.deltaPrimer =  param.deltaVV; % Death rate of infected cells [1/month] ;
PA.alphaPrimer =  param.alphaVV;  %Dying infected cell to virion conversion [virions/cell];
PA.omegaPrimer =  param.omegaVV;   %Virion decay rate [1/month].
PrimerDose = 1e2; %Amount of Primer (VV) administered

%Viral parameters VSV Strain (BOOSTER)
PA.kappaBooster =  param.kappaVSV; %Infection rate [1/month]   ;
PA.deltaBooster =  param.deltaVSV; % Death rate of infected cells [1/month] ;
PA.alphaBooster =  param.alphaVSV;  %Dying infected cell to virion conversion [virions/cell];
PA.omegaBooster =  param.omegaVSV;   %Virion decay rate [1/month].
...
%Immune Parameters
PA.kp = param.kp;    % contact rate with phagocytes [1/month]
PA.kq = param.kqs;  % factor in denominator of Q phagocytosis term [Unitless] 
PA.ks = PA.kq; % factor in denominator of S phagocytosis term [Unitless]
PA.P12 = param.Psi12;    % Half effect in cytokine driven phagocyte production [ng/mL]
...
%% Set prime and booster (enhancer and VSV) dosing
...
TimeSeries = linspace(0,tf,time_resolution); %Create 1000 evenly space points between 0 and tf
