...
function online_picard_rwm_tails(V, x0::Vector{Float64}, h::Float64, K::Int64, err::Float64)
    d = length(x0)
    Z = randn(d, K)*h
    U = rand(K)
    W = Innovation(Z, U)
    online_picard_tails(V, x0, K, W, err)
end
...
plot(scales, g, ylabel = L"L^{(1)}", marker = :circ, label = "", xlabel = L"\|x_0 - x^\star \|", title = "Convergence in the tails", ylims = (0,210))
