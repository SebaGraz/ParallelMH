...
srcdir = "/Users/zanella/Documents/bocconi/PinT/ParallelMH/out/linear/"
df = DataFrame(CSV.File(srcdir*"rwm_lin_r_d.csv"; header=false))
g1 = Matrix(df)
...
df = DataFrame(CSV.File(srcdir*"rwm_lin_r_k.csv"; header=false))
g2 = Matrix(df)
...
p = plot(p1,p2, plot_title = "Linear regression - Online-Picard RWM", size=(800,400),  bottom_margin = 3mm,  left_margin = 3mm)
savefig("lin_rwm.pdf")