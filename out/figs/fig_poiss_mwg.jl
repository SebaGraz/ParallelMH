...
srcdir = "/Users/zanella/Documents/bocconi/PinT/ParallelMH/out/poisson/"
df = DataFrame(CSV.File(srcdir*"mwg_poisson_r_d.csv"; header=false))
g1 = Matrix(df)
...
plot!(dd, dd./4, label = L"d/4", ls=:dash)


df = DataFrame(CSV.File(srcdir*"rwm_poisson_r_k.csv"; header=false))
g2 = Matrix(df)
...
p = plot(p1,p2, plot_title = "Poisson regression - Online-Picard MwG", size=(800,400), bottom_margin = 3mm, left_margin = 3mm)
savefig("poisson_mwg.pdf")