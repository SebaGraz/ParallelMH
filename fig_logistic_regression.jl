...
filename1 = "./out_csv/logistic/log_r_d.csv"
g = readdlm(filename1, ',', Float64)

dd = [30, 50, 100, 200, 300, 400, 500, 1000]
p1 = plot(dd, g[:,1], marker = :circ, xaxis= :log, yaxis = :log , 
    xlabel = L"d", ylabel = L"\hat{G}", label = L"\bar X")
plot!(dd, g[:,2], marker = :circ, label = L"\bar X_{5\%}")
plot!(dd, g[:,3], marker = :circ, label = L"\bar X_{10\%}")
plot!(dd, g[:,4], marker = :circ, label = L"\bar X_{20\%}")
plot!(dd, g[:,5], marker = :circ, label = L"\bar X_{MwG}")
plot!(dd, sqrt.(dd), label = L"\sqrt{d}", linewidth=2, ls=:dash)
plot!(dd, dd*0.33, label = L"d/3", linewidth=2, ls=:dash)


filename2 = "./out_csv/logistic/log_r_k.csv"
g = readdlm(filename2, ',', Float64)
kk =  [5, 10, 15, 25, 50, 100, 200, 400, 600]
p2 = plot(kk, g[:,1], marker = :circ, xaxis= :log, yaxis = :log , 
xlabel = L"K", ylabel = L"\hat{G}", label = L"\bar X",legend=:topleft)
plot!(kk, g[:,2], marker = :circ, label = L"\bar X_{5\%}")
plot!(kk, g[:,3], marker = :circ, label = L"\bar X_{10\%}")
plot!(kk, g[:,4], marker = :circ, label = L"\bar X_{20\%}")
plot!(kk, g[:,5], marker = :circ, label = L"\bar X_{MwG}")
...
filename3 = "./out_csv/logistic/log_r_mm.csv"
filename4 = "./out_csv/logistic/log_r_ss.csv"
mm = readdlm(filename3, ',', Float64)
ss = readdlm(filename4, ',', Float64)
err = 0.0:0.05:0.2
p3 = plot(err, mm[:, 1], ylims = (0.0, 0.2), marker = :circ, xlabel = L"r", label = L"\mathcal{M}_r"*" RWM",legend=:bottomright)
...
savefig("./logistic_regression.pdf")